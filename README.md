...
- 🚀 Built-in page caching system for improved performance
...
2. **Set Permissions**
   ```bash
   chmod -R 755 .
   chmod -R 777 content uploads feeds config cache
   ```
...
6. **Caching Layer**: Rendered pages are cached for improved performance (if enabled)
7. **Output**: Final HTML is served to the browser
...
- **Cache Storage**: Rendered pages cached in `/cache` directory
...
├── cache/                # Page cache storage (writable)
...
Forma CMS includes a flexible caching system to improve site performance by storing rendered HTML pages as static files.

### How Caching Works

1. **Page Caching**: When a page is first requested, the rendered HTML is saved to the `/cache` directory
2. **Cache Serving**: Subsequent requests serve the cached version if it's still valid
3. **Cache Expiration**: Cached pages expire after a configurable duration
4. **Smart Invalidation**: Cache is automatically cleared when content is updated
5. **Selective Caching**: Certain pages can be excluded from caching

### Cache Configuration

Access cache settings in the admin panel under **Settings > Cache**:

- **Enable Page Caching**: Toggle the caching system on/off
- **Cache Duration**: Set how long pages should be cached before expiring (in seconds)
  - Example: 3600 = 1 hour, 86400 = 24 hours
- **Excluded Paths**: Specify pages that should never be cached
  - Example: `/contact-form`, `/admin`, `/api/*`
- **Cache Strategy**: Configure when to rebuild cache automatically

### Cache Management Tools

The admin interface provides comprehensive cache management:

- **Clear Cache**: Remove all cached files to force regeneration
- **Rebuild Cache**: Clear existing cache and pre-generate common pages
- **Cache Statistics**: View cache size, file count, and hit rates
- **Cache Status**: Monitor which pages are cached and when they expire

### Performance Benefits

- **Faster Load Times**: Cached pages load instantly without processing
- **Reduced Server Load**: Less CPU and memory usage for repeat visitors
- **Better SEO**: Faster sites rank better in search engines
- **Improved User Experience**: Instant page loads improve user satisfaction
...
│   ├── about.html          # Cached about page
│   └── contact.html        # Cached contact page
├── blog/
│   ├── archive.html        # Cached blog archive
│   └── posts/
│       ├── post-1.html     # Individual cached posts
│       └── post-2.html
└── .cache-info.json        # Cache metadata and statistics
```
...
```json
{
  ...
  "cache": {
    "enabled": true,
    "duration": 3600,
    "excluded_paths": ["/contact-form", "/admin"]
  }
}
```
...
5. **Page Caching System**
   - Full-page HTML caching for static content
   - Configurable cache duration and exclusions
...
5. Consider cache invalidation requirements
...
1. **Permissions Problems**
   - Ensure write access to `content/`, `uploads/`, `feeds/`, `config/`, `cache/`
...
4. **Cache Issues**
   - Clear cache if content appears outdated
   - Check cache directory permissions
   - Verify excluded paths configuration
